...
export default {
    ...
    getLayersNum(){
      ...
      d3.selectAll(".symbol").each(function(d, i) {
        layer.push(that.getElmPath(d3.select(this)))
      })
      return Math.max(...layer)
    },
    getElmPath(elm){
      let res = []
      res.push(elm.attr('id'))
      let parent = elm
      for(let i=0; i < 20; i++){
    ...
    bindPointEvent(elm, text){
      elm.attr("pointer-events", "all")
        .on("mouseenter", (event, d) => {
          elm.append('rect').attr("class", "tip").attr('x',0).attr('y',-100).attr('width', 100).attr('height', 100).attr('fill', "#E6E6FADD")
          elm.append('text').attr("class", "tip").attr('x',0).attr('y',-50).style('fill', "#000000").text(text)
        })
        .on("mouseleave", (event, d) => {
          elm.selectAll('.tip').remove()
          d3.select('.check_box').remove() 
        })
    },
